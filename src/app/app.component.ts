...
import {
  ...
  CreateEdgeDialogInput,
  CreateEdgeDialogOutput,
} from './create-edge-dialog/create-edge-dialog.component';
import {
  CreateNodeDialogComponent,
  CreateNodeDialogInput,
  CreateNodeDialogOutput,
} from './create-node-dialog/create-node-dialog.component';
...
const EDIT_MENUS = ['create-node', 'add-edge', 'set-parent', 'remove'];
...
function syncClasses(
  elt: cytoscape.CollectionReturnValue,
  classes: Immutable.Set<string>,
) {
  const oldClasses = new Set(
    elt.classes().filter((c) => c.startsWith('user_data_')),
  );
...
function updateMenuVisibility(
  menus: contextMenus.ContextMenu,
  editMode: boolean,
): void {
  if (editMode) {
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  private data: {
    ...
    title: 'Sample graph',
    description:
      'This graph is a basic demonstration of what the app can do.\n\n' +
  ...
  private originalGraph = Graph.deserialize<Value>(
  ...
  constructor(private dialog: MatDialog) {
    ...
    const dataSource =
      new URL(location.toString()).searchParams.get('graphUrl') ||
      'assets/example.json5';
  ...
  reload(data: string) {
    ...
    this.transformations = (parsed.transformations || []).map(
      ({ name, query }) => createTransformation(name, query),
    );
    this.transformedGraph = this.transform(this.originalGraph);
  ...
  private init() {
      ...
      menuItems: [
        {
          ...
          onClickFunction: (e: cytoscape.EventObject) => {
            ...
            dialogRef
              .afterClosed()
              .subscribe((result: CreateNodeDialogOutput) => {
                if (result) {
                  ...
                  this.originalGraph = this.originalGraph.createNode(
                    ...
                    [
                      ...
                      ['position', numberList([position.x, position.y])],
                    ],
                  );
                  this.transformGraph();
                  this.updateCustomData();
        ...
        },
        {
          ...
          onClickFunction: (e: cytoscape.EventObject) => {
            this.addTransformation(
              `hide node ${e.target.id()}`,
              `MATCH (n {_ID: ${JSON.stringify(e.target.id())}}) DELETE n`,
            );
          },
        },
        {
          ...
          onClickFunction: (e: cytoscape.EventObject) => {
            this.addTransformation(
              `hide node ${e.target.id()} and descendants`,
              `MATCH ({_ID: ${JSON.stringify(e.target.id())}})<-[:_PARENT]-*(n) DELETE n`,
            );
          },
        },
        {
          ...
          onClickFunction: (e: cytoscape.EventObject) => {
            this.addTransformation(
              `hide edge ${e.target.id()}`,
              `MATCH ()-[e {_ID: ${JSON.stringify(e.target.id())}}]-() DELETE e`,
            );
          },
        },
        {
        ...
        },
        {
          ...
          onClickFunction: (e: cytoscape.EventObject) => {
            ...
            cy.nodes().once('click', (e2: cytoscape.EventObject) => {
              ...
              this.addTransformation(
                `show only nodes between ${sourceID} and ${targetID}`,
        ...
        },
        {
          ...
          onClickFunction: (e: cytoscape.EventObject) => {
            ...
            cy.nodes().once('click', (e2: cytoscape.EventObject) => {
              ...
              const dialogRef = this.dialog.open(CreateEdgeDialogComponent, {
                data: input,
              });
              dialogRef
                .afterClosed()
                .subscribe((result: CreateEdgeDialogOutput) => {
                  if (result) {
                    const classes = parseClasses(result.classes);
                    this.cachedClasses = undefined;
        ...
        },
        {
          ...
          onClickFunction: (e: cytoscape.EventObject) => {
            ...
            cy.nodes().once('click', (e2: cytoscape.EventObject) => {
              ...
              for (const [edge, dst] of this.originalGraph.outgoingNeighbors(
                node,
              )) {
                if (edge.labels.has(PARENT_LABEL)) {
                  if (dst === parent) {
    ...
    const updaters = [
      ...
      new MenuUpdater(menus, 'collapse-class', (cls) =>
        this.collapse(`.${cls}`),
      ),
      new MenuUpdater(menus, 'hide-node-class', (cls) =>
        this.addTransformation(
          `hide node class ${cls}`,
          `MATCH (n:${quoteIdentifier(cls)}) DELETE n`,
        ),
      ),
      new MenuUpdater(menus, 'hide-edge-class', (cls) =>
        this.addTransformation(
          `hide edge class ${cls}`,
          `MATCH ()-[e:${quoteIdentifier(cls)}]-() DELETE e`,
        ),
      ),
      new MenuUpdater(menus, 'hide-node-class-negated', (cls) =>
        this.addTransformation(
          `hide nodes except class ${cls}`,
          `MATCH (n:!${quoteIdentifier(cls)}) DELETE n`,
        ),
      ),
      new MenuUpdater(menus, 'hide-edge-class-negated', (cls) =>
        this.addTransformation(
          `hide edges except class ${cls}`,
          `MATCH ()-[e:!_VIRTUAL&!${quoteIdentifier(cls)}]-() DELETE e`,
        ),
      ),
      new MenuUpdater(menus, 'hide-class-simple-node-negated', (cls) =>
        this.addTransformation(
          `hide simple nodes except class ${cls}`,
          `MATCH (n:!${quoteIdentifier(cls)}) WHERE NOT (n)<-[:_PARENT]-() DELETE n`,
        ),
      ),
    ];
    this.menus = menus;
  ...
  private uncollapsedAncestor(
    nodeId: string,
  ): cytoscape.NodeCollection | undefined {
    const cy = this.cy;
    if (!cy) {
      return undefined;
    }
  ...
  private generateLayout(
    ...
    for (const edge of graph.edges) {
      if (edge.labels.has(ALIGN_VERTICAL_LABEL)) {
        const src = initial
          ? edge.srcID
          : this.uncollapsedAncestor(edge.srcID)?.data('id');
        const dst = initial
          ? edge.dstID
          : this.uncollapsedAncestor(edge.dstID)?.data('id');
        if (src && dst && src !== dst) {
  ...
  private removeIdsFromAlignmentConstraint(
    constraint: string[][],
    removed: Set<string>,
  ): string[][] {
    return constraint
  ...
  onLabelChange(label: string) {
    ...
    if (selected) {
      if (selected.source().length > 0) {
        ...
        if (edge) {
          this.originalGraph = this.originalGraph.setEdgeProperties(
            edge.id,
            edge.properties.set('label', stringValue(label)),
          );
        }
      } else {
        const node = this.originalGraph.getNodeByID(selected.id());
        if (node) {
          this.originalGraph = this.originalGraph.setNodeProperties(
            node.id,
            node.properties.set('label', stringValue(label)),
          );
        }
      }
    }
    ...
    if (selected) {
      if (selected.source().length > 0) {
        ...
        if (edge) {
          this.originalGraph = this.originalGraph.setEdgeProperties(
            edge.id,
            edge.properties.set('description', stringValue(description)),
          );
        }
      } else {
        const node = this.originalGraph.getNodeByID(selected.id());
        if (node) {
          this.originalGraph = this.originalGraph.setNodeProperties(
            node.id,
            node.properties.set('description', stringValue(description)),
          );
        }
      }
    }
    ...
    for (const selected of this.selection ?? []) {
      if (selected.source().length > 0) {
        ...
        if (edge) {
          this.originalGraph = this.originalGraph.setEdgeLabels(
            selected.id(),
            edge.labels.withMutations((labels) => {
        ...
        if (node) {
          this.originalGraph = this.originalGraph.setNodeLabels(
            selected.id(),
            node.labels.withMutations((labels) => {
              for (const c of classes) {
                labels.add(c);
              }
            }),
  ...
  addTransformation(name: string, query: string): void {
    this.transformations = [
      ...this.transformations,
      createTransformation(name, query, true),
    ];
