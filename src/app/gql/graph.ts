...
function serializeProperties<Value>(properties: Immutable.Map<string, Value>, serializeValue: (v: Value) => string): SerializedProperties {
...
let newGraphMutation: <Value>(nodes: Immutable.Map<string, Node<Value>>, edges: Immutable.Map<string, Edge<Value>>) => GraphMutation<Value>;
export class GraphMutation<Value> {
    static {
        newGraphMutation = <Value>(nodes: Immutable.Map<string, Node<Value>>, edges: Immutable.Map<string, Edge<Value>>) => new GraphMutation(nodes, edges);

    }

    private constructor(readonly nodes: Immutable.Map<string, Node<Value>>, readonly edges: Immutable.Map<string, Edge<Value>>) {}

    createNode(
        id: string,
        labels: string[]|Immutable.Set<string> = Immutable.Set(),
        properties: Array<[string, Value]>|Immutable.Map<string, Value> = Immutable.Map()
    ): this {
        if (this.nodes.has(id)) {
            throw new Error(`Node with ID ${JSON.stringify(id)} already exists`);
        }
        const node = createNode(id, Immutable.Set(labels), Immutable.Map(properties), Immutable.Set(), Immutable.Set());
    ...
    createEdge(
        ...
        const edge = createEdge(id, srcID, dstID, Immutable.Set(labels), Immutable.Map(properties));
        this.nodes.set(src.id, nodeWithOutgoingEdgeIDs(src, src.outgoingEdgeIDs.add(id)))
            .set(dst.id, nodeWithIncomingEdgeIDs(dst, dst.incomingEdgeIDs.add(id)));
        this.edges.set(id, edge);
        return this;
    }

    updateNodeLabels(nodeID: string, update: (properties: Immutable.Set<string>) => Immutable.Set<string>): this {
    ...
    updateEdgeLabels(edgeID: string, update: (properties: Immutable.Set<string>) => Immutable.Set<string>): this {
    ...
    removeNode(nodeID: string): this {
        ...
        for (const edgeID of node.incomingEdgeIDs) {
            ...
            if (edge) {
                ...
                this.nodes.set(src.id, nodeWithOutgoingEdgeIDs(src, src.outgoingEdgeIDs.delete(edgeID)));
            }
        }
        for (const edgeID of node.outgoingEdgeIDs) {
            ...
            if (edge) {
                ...
                this.nodes.set(dst.id, nodeWithIncomingEdgeIDs(dst, dst.incomingEdgeIDs.delete(edgeID)));
            }
        }
    ...
    removeEdge(edgeID: string): this {
        ...
        const dst = this.nodes.get(edge.dstID)!;
        this.nodes.set(src.id, nodeWithOutgoingEdgeIDs(src, src.outgoingEdgeIDs.delete(edgeID)))
            .set(dst.id, nodeWithIncomingEdgeIDs(dst, dst.incomingEdgeIDs.delete(edgeID)));
        return this;
    }
}
...
export class Graph<Value> {
    ...
    private constructor(nodes: Immutable.Map<string, Node<Value>>, edges: Immutable.Map<string, Edge<Value>>) {
    ...
    createEdge(
        ...
        return this.withMutations(m => m.createEdge(id, srcID, dstID, labels, properties));
    }
    ...
    static deserialize<Value>(graph: SerializedGraph, deserializeValue: (s: string) => Value): Graph<Value> {
        return Graph.new<Value>().withMutations(m => {
            for (const node of graph.nodes) {
                const properties: Array<[string, Value]> = Object.entries(node.properties ?? {})
                    .map(([k, v]) => [k, deserializeValue(v)]);
                m.createNode(node.id, Immutable.Set(node.labels ?? []), properties);
            }
            for (const edge of graph.edges) {
                const properties: Array<[string, Value]> = Object.entries(edge.properties ?? {})
                    .map(([k, v]) => [k, deserializeValue(v)]);
                m.createEdge(edge.id, edge.source, edge.target, (edge.labels ?? []), properties);
            }
        });
    }
    ...
    setNodeLabels(nodeID: string, labels: Immutable.Set<string>): Graph<Value> {
        ...
        return new Graph(this._nodesByID.set(nodeID, nodeWithLabels(node, labels)), this._edgesByID);
    }
    ...
    setEdgeLabels(edgeID: string, labels: Immutable.Set<string>): Graph<Value> {
        ...
        return new Graph(this._nodesByID, this._edgesByID.set(edgeID, edgeWithLabels(edge, labels)));
    }

    setNodeProperties(nodeID: string, properties: Immutable.Map<string, Value>): Graph<Value> {
        const node = this._nodesByID.get(nodeID);
        if (!node) {
            throw new Error(`Node not found: ${JSON.stringify(nodeID)}`);
        }
        return new Graph(this._nodesByID.set(nodeID, nodeWithProperties(node, properties)), this._edgesByID);
    }

    setEdgeProperties(edgeID: string, properties: Immutable.Map<string, Value>): Graph<Value> {
        const edge = this._edgesByID.get(edgeID);
        if (!edge) {
            throw new Error(`Edge not found: ${JSON.stringify(edgeID)}`);
        }
        return new Graph(this._nodesByID, this._edgesByID.set(edge.id, edgeWithProperties(edge, properties)));
    }
...
function nodeWithIncomingEdgeIDs<V>(node: Node<V>, ids: Immutable.Set<string>): Node<V> {
    return createNode<V>(node.id, node.labels, node.properties, ids, node.outgoingEdgeIDs);
}

function nodeWithOutgoingEdgeIDs<V>(node: Node<V>, ids: Immutable.Set<string>): Node<V> {
    return createNode<V>(node.id, node.labels, node.properties, node.incomingEdgeIDs, ids);
}

function nodeWithLabels<V>(node: Node<V>, labels: Immutable.Set<string>): Node<V> {
    return createNode(node.id, labels, node.properties, node.incomingEdgeIDs, node.outgoingEdgeIDs);
}

function nodeWithProperties<V>(node: Node<V>, properties: Immutable.Map<string, V>): Node<V> {
    return createNode(node.id, node.labels, properties, node.incomingEdgeIDs, node.outgoingEdgeIDs);
}

function edgeWithLabels<V>(edge: Edge<V>, labels: Immutable.Set<string>): Edge<V> {
    return createEdge(edge.id, edge.srcID, edge.dstID, labels, edge.properties);
}

function edgeWithProperties<V>(edge: Edge<V>, properties: Immutable.Map<string, V>): Edge<V> {
