...
import {formatExpression, formatPath, parseQuery, parseEdge, parseExpression, parseNode, Path} from './parser';

describe('cypher', () => {
    describe('parseQuery', () => {
        it('parses a match', () => {
            expect(parseQuery('match (a)--(b)-[x]->(c)\ndetach delete x\ndelete y')).toEqual(jasmine.objectContaining({
                reads: [{
                    kind: 'match',
                    paths: [{
                        nodes: [{
        ...
        it('parses a match with multiple paths', () => {
            expect(parseQuery('match (a), (b) return a, b')).toEqual(jasmine.objectContaining({
                reads: [{
                    kind: 'match',
                    paths: [{
                        nodes: [{
    ...
    describe('parseNode', () => {
        it('parses empty', () => {
            expect(parseNode('()')).toEqual(jasmine.objectContaining({
                name: null,
                label: null,
            }));
        });
        ...
        it('parses identifier only', () => {
            expect(parseNode('(a)')).toEqual(jasmine.objectContaining({
                name: 'a',
                label: null,
            }));
        });
        ...
        it('parses label only', () => {
            expect(parseNode('(:Foo)')).toEqual(jasmine.objectContaining({
                name: null,
                label: {
                    kind: 'identifier',
                    value: 'Foo',
        ...
        it('parses label expressions', () => {
            expect(parseNode('(:a&b&c)')).toEqual(jasmine.objectContaining({
                name: null,
                label: {
                    kind: 'conjunction',
                    values: [{
        ...
        it('parses identifier and label', () => {
            expect(parseNode('(a:Foo)')).toEqual(jasmine.objectContaining({
                name: 'a',
                label: {
                    kind: 'identifier',
                    value: 'Foo',
        ...
        it('parses properties', () => {
            ...
            expect(parseNode('({abc: 123})')).toEqual(jasmine.objectContaining({
                properties: [['abc', {kind: 'number', value: 123}]],
            }));
            expect(parseNode('({abc: 123, def: 456})')).toEqual(jasmine.objectContaining({
                properties: [['abc', {kind: 'number', value: 123}], ['def', {kind: 'number', value: 456}]],
            }));
            expect(parseNode('({abc: "xyz"})')).toEqual(jasmine.objectContaining({
        ...
        it('parses identifier and properties', () => {
            expect(parseNode('(x {abc: 123})')).toEqual(jasmine.objectContaining({
                name: 'x',
                properties: [['abc', {kind: 'number', value: 123}]],
            }));
        });
    ...
    describe('parseEdge', () => {
        it('parses emptier undirected', () => {
            expect(parseEdge('--')).toEqual(jasmine.objectContaining({
                name: null,
                label: null,
                direction: 'NONE',
            }));
        ...
        it('parses identifier only', () => {
            expect(parseEdge('-[a]-')).toEqual(jasmine.objectContaining({
                name: 'a',
                label: null,
            }));
        });
        ...
        it('parses label only', () => {
            expect(parseEdge('-[:Foo]-')).toEqual(jasmine.objectContaining({
                name: null,
                label: {
                    kind: 'identifier',
                    value: 'Foo',
        ...
        it('parses identifier and label', () => {
            expect(parseEdge('-[a:Foo]-')).toEqual(jasmine.objectContaining({
                name: 'a',
                label: {
                    kind: 'identifier',
                    value: 'Foo',
        ...
        it('parses properties', () => {
            expect(parseEdge('-[{abc: 123}]-')).toEqual(jasmine.objectContaining({
                name: null,
                properties: [['abc', {kind: 'number', value: 123}]],
            }));
        });
    ...
    describe('parseExpression', () => {
        it('parses ascii identifier', () => {
            expect(parseExpression('abc_2')).toEqual({kind: 'identifier', value: 'abc_2'});
        });

        it('parses quoted identifier', () => {
            expect(parseExpression('`abc 123``\\u0040`')).toEqual({kind: 'identifier', value: 'abc 123`@'});
        });
