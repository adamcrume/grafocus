  ...
  for (const element of elements) {
    ...
    while (elt.data.parent) {
      if (elt.data.parent === element.data.id) {
        throw new Error(
          `Parent cycle detected involving element ${element.data.id}`,
        );
      }
      const parent = elementsByID.get(elt.data.parent);
      if (!parent) {
        throw new Error(
          `Parent ${elt.data.parent} of element ${elt.data.id} not found`,
        );
      }
      elt = parent;
    }
    if (element.data.source) {
      if (!elementsByID.has(element.data.source)) {
        throw new Error(
          `Source ${element.data.source} of edge ${element.data.id} not found`,
        );
      }
    }
    if (element.data.target) {
      if (!elementsByID.has(element.data.target)) {
        throw new Error(
          `Target ${element.data.target} of edge ${element.data.id} not found`,
        );
      }
    }
  }
